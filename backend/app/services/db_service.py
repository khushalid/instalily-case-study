...
from app.core.config import settings, embedding_client
...
def construct_partselect_search_url(query_term: str) -> str:
    """
    Constructs a URL for searching on PartSelect.com's external search endpoint.

    Args:
        query_term (str): The term to be searched.

    Returns:
        str: The fully formed search URL.
    """
...
def get_db_connection():
    ...
    Returns:
        psycopg2.connection | None: A database connection object, or None if connection fails.
    """
    try:
...
def get_part_details(part_number: str) -> dict:
    ...
    Args:
        part_number (str): The PartSelect part number (e.g., 'PS11752778').

    Returns:
        dict: A dictionary with 'found' status, 'details' (if found), and potential messages/search URLs.
    """
    conn = None
    try:
...
def check_compatibility(part_number: str, model_number: str) -> dict:
    ...
    Returns:
        dict: Compatibility status and relevant information.
    """
    conn = None
    try:
        conn = get_db_connection()
...
def troubleshoot_appliance(appliance_type: str, symptom: str) -> dict:
    ...
    Performs a basic keyword search against troubleshooting guides.
...
def get_installation_guide_by_part(part_number: str, guide_type: str = 'Installation') -> dict:
    """
    Retrieves installation or troubleshooting guides for a specific part or model.

    Args:
        part_number (str, optional): The PartSelect part number.
        model_number (str, optional): The appliance model number.
        guide_type (str): Type of guide ('Installation', 'Troubleshooting', 'Repair', 'Guide').
...
def get_query_embedding(text: str) -> list:
    ...
    Args:
        text (str): The input text query to embed.

    Returns:
        list | None: A list of floats representing the embedding, or None if embedding fails.
    """
...
def semantic_search(query: str, appliance_type: str = None, search_limit: int = 5) -> dict:
    ...
    Returns:
        dict: A dictionary containing semantically similar parts and guides,
              along with a status message and potential search URL.
    """
